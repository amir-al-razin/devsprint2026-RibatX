...
**Core Philosophy:** *Survival over Speed.* A failure in one service must never cascade to bring down the entire system.
...
### Day 2 — Identity Provider + Order Gateway Core
...
- [ ] Prisma schema (`Student` model), run `prisma migrate dev`
- [ ] `AuthModule`: `POST /auth/register` (hash password with bcrypt), `POST /auth/login` (compare hash, issue JWT)
- [ ] JWT module setup: `@nestjs/jwt`, sign with `JWT_SECRET` env var, 1h expiry
- [ ] Rate limiting middleware: Redis INCR counter `rate_limit:login:{studentId}`, reject with 429 if > 3

**Order Gateway (`apps/gateway`):**
- [ ] `JwtAuthGuard` using `passport-jwt`: validates Bearer token on protected routes
- [ ] `POST /orders` route stub (returns 202 placeholder)
- [ ] `GET /health` stub
- [ ] Forward JWT validation to `@nestjs/passport` strategy (no DB call needed — just verify signature)

**Shared:**
- [ ] Add `JWT_SECRET` to all `.env` files and `docker-compose.yml` environment section
...
### Day 3 — Stock Service + Redis Cache
...
- [ ] Prisma schema (`Item` model with `version` field), seed 1 item (e.g., "Iftar Box", qty: 100)
...
- [ ] `GET /items` and `GET /items/:id` endpoints
- [ ] Unit tests: `stock.service.spec.ts` (mock Prisma, test version conflict, test zero-stock path)

**Order Gateway — Cache Layer:**
- [ ] On `POST /orders`: check `GET stock:{itemId}` from Redis
...
- [ ] Track `cache_hits` and `cache_misses` counters in-memory (exposed via `/metrics`)
- [ ] Wire up `POST /orders` to call `POST http://stock:3002/reserve` via `HttpModule`
...
### Day 4 — Kitchen Queue + Notification Hub
...
- [ ] Install `@nestjs/bull` + `bull` (or `bullmq`)
- [ ] On successful stock reservation: `kitchenQueue.add('process', { orderId, studentId, itemId })`
- [ ] Return `202 Accepted { orderId, status: 'PENDING' }` to client immediately

**Kitchen Queue (`apps/kitchen`):**
- [ ] BullMQ Worker: `@Process('process')` handler
- [ ] Step 1: `PATCH http://notification:3004/notify/:orderId` with `{ status: 'IN_KITCHEN' }`
- [ ] Step 2: `await sleep(random(3000, 7000))`
- [ ] Step 3: `PATCH http://notification:3004/notify/:orderId` with `{ status: 'READY' }`
- [ ] `GET /queue/length` → returns current BullMQ waiting job count

**Notification Hub (`apps/notification`):**
- [ ] Socket.io Gateway: `@WebSocketGateway({ cors: true })`
- [ ] On connect: client emits `join` event with `studentId` → server does `socket.join('student:{studentId}')`
- [ ] `PATCH /notify/:orderId` HTTP endpoint: looks up `studentId` for order → `io.to('student:{studentId}').emit('order:status', { orderId, status })`

**Idempotency (Gateway):**
- [ ] Store `X-Idempotency-Key` → `orderId` mapping in Redis (24h TTL)
- [ ] On duplicate key: return the cached response without re-processing
