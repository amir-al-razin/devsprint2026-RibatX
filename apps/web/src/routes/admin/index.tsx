...
const GATEWAY_METRICS_URL = `${env.VITE_GATEWAY_URL ?? '/api'}/metrics`
...
function HealthDot({ url }: { url: string }) {
  ...
  const label =
    Object.keys(HEALTH_URLS).find((k) => HEALTH_URLS[k] === url) ?? url
  return (
      ...
      <section>
        ...
        <Card size="sm">
          <CardContent className="pt-4 flex flex-wrap gap-4">
            {Object.entries(HEALTH_URLS).map(([, url]) => (
              <HealthDot key={url} url={url} />
            ))}
          </CardContent>
        </Card>
      </section>
